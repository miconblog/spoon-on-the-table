...
function authentication(req, res, next) {
  console.log('authentication...', req.method, req.originalUrl);
  ...
  query.find()
    .then((users) => {
      if (users.length === 0) {
        ...
        res.status(200).json({ message: 'Sorry, it was duplicated!' });
      }
    })
...
function updateUser(req, res) {
  ...
  if (user.id !== id) {
    return res.status(401).json({ code: 1, message: '잘못된접근입니다.' });
  }
...
function deleteUser(req, res) {
  ...
  // 권한 확인
  if (!user || (user.id !== id)) {
    return res.status(406).json({ code: 1, message: '권한이 없습니다.' });
  }
